...
if (devicechanged['Sécurité'] == 'On') then
	...
	os.execute('/home/pi/tools-domo/alarme.sh init '..uservariables['SecPanel_pincode']..' &');
else if (devicechanged['Sécurité'] == 'Off') then
	print('(Security) Passage de Armé à Désarmé.')
	os.execute('/home/pi/tools-domo/alarme.sh finish '..uservariables['SecPanel_pincode']..' &');
end
end
